    ...
    class Program
    {
        ...
        static void ConsultarCriptoDB()
        {
            using (var db = new CryptoContext())
            {
                ...
                var list1 = db.Moneda.Where(a => a.Actual > 50).OrderBy(m => m.MonedaId).ToList();
                list1.ForEach(Console.WriteLine);

                // 1

                WriteLine("2.- Carteras con más de 2 monedas contratadas");
                var list2 = db.Contrato.GroupBy(a=>a.CarteraId).Select(b=> new{
                    CarteraId=b.Key,
                    Cantidad=b.Count()
                }).Where(b=>b.Cantidad>2).ToList();

                list2.ForEach(Console.WriteLine);

                // 1
                WriteLine("3.- Exchanges ordenados por números de carteras");
                var list3 = db.Cartera.GroupBy(a=>a.Exchange).Select(b=> new{
                    CarteraId=b.Key,
                    Cantidad=b.Count()
                ...
                WriteLine("4.- Exchanges ordenados por cantidad de monedas");
                 var list4 = db.Cartera.GroupBy(a=>a.Exchange).Select(b=> new{
                    CarteraId=b.Key,
                    Cantidad=b.Count()
                }).OrderByDescending(a =>a.Cantidad).ToList();
